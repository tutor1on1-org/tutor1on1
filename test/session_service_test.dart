...
''',
  ...
  test(
      'review turn falls back to cached bundle lecture when no question bank exists',
      () async {
    final artifactRoot =
        await Directory.systemTemp.createTemp('session_bundle_artifacts_');
    final sourceRoot = await Directory.systemTemp.createTemp(
    ...
    try {
      final teacherId = await db.createUser(
        username: 'teacher_review_bundle_session',
        pinHash: 'hash',
        role: 'teacher',
        remoteUserId: 2101,
      );
      final studentId = await db.createUser(
        username: 'student_review_bundle_session',
        pinHash: 'hash',
        role: 'student',
        teacherId: teacherId,
        remoteUserId: 2102,
...
''',
      ...
      llmService.queueCall(
        Future<LlmCallResult>.value(
          _llmOk(
            responseText: jsonEncode(<String, Object?>{
              'text': 'What are the three kinds of integer signs?',
              'difficulty': 'easy',
              'mistakes': <String>[],
              'next_action': 'review',
              'finished': false,
