...
- Public GitHub source safety: before pushing to `github`, verify whether the target branch contains backend paths such as `remote/`; never keep bootstrap/admin credentials in code, and treat any previously public bootstrap password as leaked until production is rotated.
