...
- Delivery automation rule: tracked pre-push hook runs `scripts/hook_memory_update.ps1` (line-delta trigger `>10` with snapshot `scripts/memory_line_snapshot.json`) before `scripts/validate_project.ps1 -NoPostHook`; if memory files are updated, commit them before push.
