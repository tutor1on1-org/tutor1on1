...
- Sync catch-up rule: session/progress download must drain paginated deltas in one sync run (for example `limit=500` loop until short page/empty) instead of one page per 60-second cycle, or large unchanged histories can take 10+ minutes to settle.
