...
- Current public release tag: `v1.0.3`
- App version in `pubspec.yaml`: `1.0.3`
...
```powershell
powershell -ExecutionPolicy Bypass -File public_release/package_github_release.ps1 -ReleaseTag v1.0.3
```

Or publish the GitHub Release assets directly:

```powershell
powershell -ExecutionPolicy Bypass -File public_release/publish_github_release.ps1 -ReleaseTag v1.0.3
```

That script builds Android and Windows release artifacts and writes them to:

```text
public_release/dist/v1.0.3/
```
