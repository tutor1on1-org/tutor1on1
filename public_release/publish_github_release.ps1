...
function Assert-ReleaseTagExists {
  param(
    [Parameter(Mandatory = $true)]
    [string]$RepoSlug,
    [Parameter(Mandatory = $true)]
  ...
  $response = Invoke-GitHubApi `
    -Method 'GET' `
    -Url "https://api.github.com/repos/$RepoSlug/git/ref/tags/$ReleaseTag" `
    -Token $Token
  if ($response.StatusCode -eq 404) {
    throw "Public source tag does not exist; refusing to create a release from default HEAD: $ReleaseTag"
  }
  if ($response.StatusCode -lt 200 -or $response.StatusCode -ge 300) {
    throw "GitHub tag lookup failed with status $($response.StatusCode): $($response.BodyText)"
  }
}
...
try {
  if (-not $SkipPackage.IsPresent) {
    ...
    if ($UseExistingPublishedArtifacts.IsPresent) {
      $packageArgs += '-UseExistingPublishedArtifacts'
    }
