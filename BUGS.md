...
## Recent bug fixes
- 2026-07-24 Public release automation referenced deleted `public_release` GitHub publisher scripts, so the documented default flow could only finish by explicitly skipping GitHub Release assets. Fix: restore the exact publisher/package scripts and make publishing resume-safe by skipping existing assets only when GitHub's SHA-256 digest matches the local file, while replacing missing or mismatched assets. Prevention: keep the canonical release wrapper executable end to end and make every large-asset publish idempotent after interruption.
